/**
 * 궁합 결과 데이터 빌더
 * - compatibility/[sign1]/[sign2]/page.tsx
 * - [locale]/compatibility/[sign1]/[sign2]/page.tsx
 * 두 페이지에서 공유하는 로직
 */
...
const elementDescriptions: Record<string, LocalizedText> = {
...
const defaultElementDesc: LocalizedText = {
...
const diffModalityDesc: LocalizedText = {
...
function getElementDescription(e1: string, e2: string): LocalizedText {
  return elementDescriptions[`${e1}-${e2}`] || elementDescriptions[`${e2}-${e1}`] || defaultElementDesc;
}

function getModalityDescription(m1: string, m2: string): LocalizedText {
  if (m1 === m2) return sameModalityDesc[m1] || sameModalityDesc['mutable'];
  return diffModalityDesc;
}

function createCategory(score: number, cat: string) {
  return {
    score,
    description: {
...
export const zodiacKoNames: Record<string, string> = {
